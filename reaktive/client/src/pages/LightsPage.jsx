...
const LightsPage = ({ lights: initialLights = [], toggleLight: propToggleLight, setBrightness: propSetBrightness }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try { const all = await haService.discoverEntities(); const mapped = (all.lights||[]).map(mapEntityToLight); if (mounted) setLights(mapped);} catch (e) { console.warn(e); }
      try { haService.subscribeToEntity('state_changed', onStateChanged); } catch (e) { console.warn(e); }
    })();

    return () => { mounted = false; try { haService.unsubscribeFromEntity('state_changed', onStateChanged); } catch (e) {} try { haService.disconnectWebSocket(); } catch (e) {} };
  }, []);
